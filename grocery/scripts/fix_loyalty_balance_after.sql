-- =============================================================================
-- fix_loyalty_balance_after.sql
--
-- Recalculates balance_after for existing loyalty_point_transactions rows
-- to match the sequential running-sum approach expected by the dbt test
-- assert_loyalty_points_balance.
--
...
-- ordered by (transaction_dt, pt_id) using pt_id as the tiebreaker for
-- same-timestamp rows.
--
...
-- We order by (t.transaction_dt, lpt.pt_id) to match the dbt test ordering
-- (the test uses transaction_dt + points_balance_after as tiebreaker, but
-- pt_id is a stable proxy for insertion order when the original balance_after
-- values are wrong).
WITH running AS (
    SELECT
        lpt.pt_id,
...
-- This ensures the API endpoint returns the correct current balance.
WITH latest AS (
